...
@Path("/appfirmardocumentosimple")
public class ServicioAppFirmarDocumentoSimple extends RequestSizeFilter {

    private static final Logger LOGGER = Logger.getLogger(ServicioAppFirmarDocumentoSimple.class.getName());

    @POST
    @Secured
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
    public String firmarDocumentoSimple(
            @FormParam("pkcs12") String pkcs12Base64,
            @FormParam("password") String password,
            @FormParam("documento") String documentoBase64,
            @FormParam("json") String jsonMetadata
    ) {
        LOGGER.log(Level.INFO, "Iniciando firma simple");
        ...
        try {
            ...
            // 3. Configurar parametros de firma simple (information1)
            Properties params = new Properties();
            params.setProperty("typeSignature", "information1");
            ...
            response.addProperty("mensaje", "Documento firmado con firma simple exitosamente");
